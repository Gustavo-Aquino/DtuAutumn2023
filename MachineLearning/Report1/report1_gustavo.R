...
library(plotly)
# Data format ##################################################################
data <- read.csv("C:/Faculdade/Intercâmbio/Machine Learning/Glass/glass.data")
...
# Problems ####
# Problem 3
# When we apply SVD and have the matrices U V and S, the S diagonal represents the singular values
# that can lead to the variance explained when applied , looking at the values c(13.9,12.47,11.49,10.03,9.45)
# we can display a cumulative sum to check all the answers:
sing_values <- c(13.9,12.47,11.49,10.03,9.45)
explained_var <- list()
j = 0
for(i in sing_values){
  expl_var <- i^2/sum(sing_values^2)
  print(expl_var)
  j <- j+1
  explained_var[j] <- expl_var
}

# Problem 4
# Here we have to analyse the V matrix, no code necessary
